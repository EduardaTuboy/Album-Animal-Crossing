...
export const addSticker = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao adicionar figurinha no banco de dados.' });
  }
};
...
export const updateSticker = async (req, res) => {
  ...
  try {
    const updated = await Stickers.updateStickerByNumber(stickerNumber, req.body);
    res.json(updated);
  } catch (err) {
    console.error('Erro ao atualizar sticker:', err);
    res.status(500).json({ error: 'Erro ao atualizar figurinha no banco de dados.' });
  }
};
...
export const deleteSticker = async (req, res) => {
  ...
  try {
    ...
    if (!deleted) return res.status(404).json({ error: 'Villager não encontrado.' });
    res.json({ message: 'Villager eliminado com sucesso!', deleted });
  } catch (err) {
    console.error('Erro ao eliminar sticker:', err);
    res.status(500).json({ error: 'Erro ao eliminar o villager da base de dados.' });
  }
};
